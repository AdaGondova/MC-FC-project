...
### !!! this is a slightly different version which checks if parcellations already exist from the previous projects and if yes then copies them over to /networks
#############################################################################################
echo Stared at: `date`
...
do
	...
	OUTPUTDIR=/neurospin/grip/external_databases/dHCP_CR_JD_2018/Projects/networks/DerivedData/subjects/sub-${subject_id}/ses-${session_id}

	PREVIOUS=/neurospin/grip/external_databases/dHCP_CR_JD_2018/Projects/rel3_dhcp_anat_pipeline_analysis/sub-${subject_id}/ses-${session_id}/anat
	
	echo RUNNING subject "${subject_id}" session "${session_id}"


	if [ -f $PREVIOUS/sub-${subject_id}_ses-${session_id}_rh.DKT.label.gii ]
	then
		echo FILES exist already 
		mkdir -p $OUTPUTDIR
		cp $PREVIOUS/sub-${subject_id}_ses-${session_id}_rh.DKT.label.gii $OUTPUTDIR/sub-${subject_id}_ses-${session_id}_rh.DKT.label.gii
		cp $PREVIOUS/sub-${subject_id}_ses-${session_id}_lh.DKT.label.gii $OUTPUTDIR/sub-${subject_id}_ses-${session_id}_lh.DKT.label.gii

	
	else
		
		## the below should skip subjects with misssing inputs or who already have been segmented
		if [ -d "$INPUTDIR" ] || [ ! -f $OUTPUTDIR/sub-${subject_id}_ses-${session_id}_rh.DKT.label.gii ]
		then 
			...
			then 
				...
				echo ALREADY PARCELLATED  
			fi
		fi 
	fi 
	echo ${subject_id},${session_id} >> MCRIBS_finished.txt
done < "$1"
